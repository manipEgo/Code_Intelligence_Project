        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    torch.save(model_to_save.state_dict(), os.path.join(last_output_dir, "premodel.pt"))
...
def main():
    ...
    #训练过程中每经过多少个更新步骤（update steps）后，将当前的模型参数保存为一个检查点。
    ...
    #自定义 训练的数据集行数
    parser.add_argument("--train_line", type=int, default=2000,
                        help="训练的数据集行数")
    ...
    logger.info(args.no_cuda)
    if args.local_rank == -1 or args.no_cuda:
        device = torch.device("mps" if torch.cuda.is_available() and not args.no_cuda else "cpu")
        args.n_gpu = torch.cuda.device_count()
    else:  # Initializes the distributed backend which will take care of sychronizing nodes/GPUs
        torch.cuda.set_device(args.local_rank)
        device = torch.device("mps", args.local_rank)
    ...
    if pretrained:
        ...
        if args.model_type == "rnn":
            ...
            if os.path.exists(model_last):
                logger.warning("Loading model from {model_last}")
